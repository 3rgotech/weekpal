...
import { DndContext, closestCenter, DragEndEvent, DragStartEvent, DragOverlay, CollisionDetection, pointerWithin, rectIntersection, MeasuringStrategy, DragOverEvent } from "@dnd-kit/core";
...
const MainContent: React.FC<MainContentProps> = () => {
  ...
  const [activeTask, setActiveTask] = useState<{ task: Task | null, dayOfWeek: DayOfWeek | null }>({
    task: null,
    dayOfWeek: null
  });
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    if (over.data.current?.type === 'task') {
      ...
      const overTask = findTask(parseInt(over.id.toString().replace('task-', ''), 10));
      if (!overTask) return;
      ...
      newOrder = isBelowOverItem ? (overTask.order ?? 0) + 1 : (overTask.order ?? 0);
    } else {
