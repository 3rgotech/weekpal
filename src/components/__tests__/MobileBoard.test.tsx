...
describe("the board on a phone", () => {
    ...
    it("draws every row of an ordinary column, exactly as before", () => {
        // The threshold matters as much as the windowing: a normal board must not pay for this.
        const few = Array.from({ length: 12 }, (_, i) => weekly(`Task ${i}`, today));
        data.tasks = few;
        data.allTasks = few;
    ...
    it("windows a long column instead of laying all of it out", () => {
        // Measured before this existed: a thousand rows in one column was 11,374 DOM nodes and a
        // forced-layout loop of 879 ms. jsdom has no scroll height, so the window here is empty —
        // what this asserts is that the column switched to the windowed path at all.
        const many = Array.from({ length: 200 }, (_, i) => weekly(`Task ${i}`, today));
        data.tasks = many;
        data.allTasks = many;
        ...
        expect(screen.queryAllByText(/^Task \d+$/).length).toBeLessThan(many.length);
    });
